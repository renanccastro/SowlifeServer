...
module.exports.getAllPrays = function(request, res){
	var decoded = jwt.decode(request.headers.token, tokenSecret);
	...
	var userid = utils.sanitizedUserID(decoded.userid);
	request.db.collection('pray_requests').find({
		$or: [{
			"SharedWith": userid
		},
		{
			"Author" : userid
		}]}, {"sort":[["CreationDate","desc"]]}).toArray(function(err, records) {
			res.json(records);
	});
}

module.exports.getPrayById = function(request, res){
	var decoded = jwt.decode(request.headers.token, tokenSecret);
	...
	var userid = utils.sanitizedUserID(decoded.userid);
	request.db.collection('pray_requests').findOne({
		"_id": utils.sanitizedUserID(request.params.id)
		},function(er, pray) {
			res.json(pray)
	})
}
